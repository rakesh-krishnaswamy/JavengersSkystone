...
@Autonomous(name = "BlueAutoNoFoundation4Stone", group = "Concept")
public class BlueAutoNoFoundation4Stone extends LinearOpMode {
...
//            autoLib.calcTurn(3, slowPower);
            autoLib.calcMove(10, fastPower, Constants.Direction.LEFT);    // move back little
//            autoLib.calcTurn(5, slowPower);    // turn, so that the robot will go straight
            autoLib.rampMove(185, 1f, Constants.Direction.FORWARD, true);  // move forward towards foundation
            ...
            if (distance > foundationDistance) {
                autoLib.calcMove((float) (distance - foundationDistance), mediumPower, Constants.Direction.RIGHT);
            }
